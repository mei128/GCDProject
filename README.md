...
```
### Step 2 ###
We trim the data set by selecting only Subject, Activity, and those columns which contain mean and standard deviation features.  After the *make.names* statement, 'mean()' became 'mean..', and 'std()' became 'std..', hence the trailing '\\.' for both in the *select* statement. Note that only features with mean() or stdev() in the name were considered, ignoring others like 'fBodyAcc-meanFreq()-X'.
```{r eval=FALSE}
